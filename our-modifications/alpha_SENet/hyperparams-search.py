...
def main(**kwargs):
    ...
    for idx, model_idx in enumerate(opt.model):
        for lrate in opt.learning_rate:
            for wdecay in opt.weight_decay:
                print(f"Running: {model_idx}, idx={idx}")
                print(f"LR: {lrate}, BS: {opt.batch_size}, WD: {wdecay}")
                ...
                device = t.device(
                    opt.device
                    if (t.cuda.is_available() and opt.gpu_mode >= 1)
                    else "cpu"
                )
                print(f"On Server: {device}")
                t.backends.cudnn.benchmark = True

                dst_str = f"{opt.time_stamp}-{model_idx}-nc_{opt.num_classes}-ME_{opt.max_epoch}-LR_{opt.use_lr_scheduler}_{lrate}-BS_{opt.batch_size}-WD_{wdecay}"
                # loadng datasets
                data_transform = {
                    "train": T.Compose(
                        [
                ...
                if opt.continue_training:
                    ...
                    assert os.path.exists(
                        model_weight_path
                    ), "cannot find {} file".format(model_weight_path)
                ...
                params_groups = get_params_groups(model, weight_decay=wdecay)
                ...
                if opt.use_earlystop:
                    earlystop = EarlyStopping(opt.earlystop_patience)  # init earlystop
                ...
                for epoch in range(opt.max_epoch):
                    ...
                    for step, (images, labels, _, _) in enumerate(train_loader_bar):
                        ...
                        train_loader_bar.desc = "[train epoch {}] loss: {:.3f}, acc: {:.3f}, lr: {:.7f}".format(
                            epoch,
                    ...
                    with t.no_grad():
                        ...
                        for val_step, (val_images, val_labels, _, _) in enumerate(
                            ...
                            val_accu_num += t.eq(
                                val_pred_classes, val_labels.to(device)
                            ).sum()
                    ...
                    writer.add_scalar(
                        accu_tags[0], accu_loss.item() / (step + 1), epoch + 1
                    )
                    writer.add_scalar(
                        accu_tags[2], accu_num.item() / sample_num, epoch + 1
                    )
                    writer.add_scalar(
                        accu_tags[1],
                        val_accu_loss.item() / (val_step + 1),
                    ...
                    if epoch >= 5:
                        t.save(
                            model.state_dict(),
                            f"{checkpoint_dst}/last_model-{epoch+1}.pth",
                        )
