...
def plot_data_loader_image(data_loader):
    from data_config.constants import (
        SMALL_XHGNET_DEFAULT_MEAN,
        SMALL_XHGNET_DEFAULT_STD,
    )
    ...
    for data in data_loader:
        ...
        for i in range(plot_num):
            ...
            # 反Normalize操作
            img = (img * SMALL_XHGNET_DEFAULT_STD + SMALL_XHGNET_DEFAULT_MEAN) * 255
            label = labels[i].item()
...
def get_params_groups(
    model: torch.nn.Module, weight_decay: float = 1e-5, verbose: bool = False
):
    """No bias decay optimization.
    Paper: `"Bag of tricks" <https://arxiv.org/abs/1812.01187>`
    As pointed out by *Jia et al., it's recommended to only apply the
    regularization to weights to avoid overfitting.
    """
    # 记录optimize要训练的权重参数
